...
<div class="flex justify-center">
    <form class="w-full " method="POST" action="{{ route('users.store') }}">
        ...
        <div class="w-full px-3">
            <label for="user_type" class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">{{ __('Tipo de Usuario') }}</label>

            <div class="col">
                <select class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 @error('email') is-invalid @enderror" name="user_type" id="user_type">
                    @foreach (App\Models\UserType::all() as $item)
                    <option value="{{$item->id}}">{{ucfirst(__($item->name))}}</option>
                    @endforeach
                </select>
